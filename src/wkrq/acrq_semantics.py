...
class ACrQInterpretation:
    ...
    def set_bilateral(self, predicate: str, args: tuple[str, ...], 
                      positive: TruthValue, negative: TruthValue) -> None:
        """Set bilateral values for a predicate with arguments."""
        # Validate bilateral consistency
        if positive == TRUE and negative == TRUE:
            raise ValueError(f"Bilateral inconsistency: {predicate}{args} cannot be both true and false")
        
        key = f"{predicate}{args}"
        self.bilateral_values[key] = BilateralTruthValue(positive=positive, negative=negative)
        
        # Add individuals to domain
        for arg in args:
            self.domain.add(arg)
    
    def get_bilateral(self, predicate: str, args: tuple[str, ...]) -> BilateralTruthValue:
...
class ACrQEvaluator:
    ...
    def __init__(self, interpretation: ACrQInterpretation):
    ...
    def _eval_with_substitution(self, formula: Formula, substitution: Dict[str, str]) -> TruthValue:
