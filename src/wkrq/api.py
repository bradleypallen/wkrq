...
from .tableau import solve, valid, entails, TableauResult, Model
from .parser import parse, parse_inference, Inference
...
def check_inference(inference: Inference, consequence: str = "strong") -> InferenceResult:
    """
...
def evaluate_formula(formula: Formula, sign: Sign = T, 
                    track_steps: bool = False) -> TableauResult:
    """
...
def find_models(formula: Formula, sign: Sign = T, limit: Optional[int] = None) -> List[Model]:
    """
...
class WkrqLogic:
    ...
    def models(self, formula: Formula, sign: str = "T", limit: Optional[int] = None) -> List[Model]:
