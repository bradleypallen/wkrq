...
class Formula(ABC):
    ...
    @staticmethod
    def restricted_exists(variable: 'Variable', restriction: 'Formula', 
                         matrix: 'Formula') -> 'RestrictedExistentialFormula':
        """Create a restricted existential quantifier: [∃X P(X)]Q(X)"""
        return RestrictedExistentialFormula(variable, restriction, matrix)
    
    @staticmethod
    def restricted_forall(variable: 'Variable', restriction: 'Formula',
                         matrix: 'Formula') -> 'RestrictedUniversalFormula':
...
class CompoundFormula(Formula):
    ...
    def __str__(self) -> str:
        if self.connective == '~' and len(self.subformulas) == 1:
            ...
            right_str = f"({right})" if isinstance(right, CompoundFormula) else str(right)
            return f"{left_str} {self.connective} {right_str}"
        else:
            raise ValueError(f"Invalid compound formula: {self.connective} with {len(self.subformulas)} subformulas")
...
class RestrictedQuantifierFormula(Formula):
    ...
    def __init__(self, quantifier: str, variable: Variable, 
                 restriction: Formula, matrix: Formula):
...
def Negation(formula: Formula) -> CompoundFormula:
    """Create a negation."""
    return CompoundFormula("~", [formula])


def Conjunction(left: Formula, right: Formula) -> CompoundFormula:
    """Create a conjunction."""
    return CompoundFormula("&", [left, right])


def Disjunction(left: Formula, right: Formula) -> CompoundFormula:
    """Create a disjunction."""
    return CompoundFormula("|", [left, right])


def Implication(left: Formula, right: Formula) -> CompoundFormula:
