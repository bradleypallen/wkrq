...
import time
from wkrq import (
    Formula, solve, valid, entails, parse, parse_inference, 
    check_inference, T, F, M, N
)
...
class TestPriestNonClassicalLogic:
    ...
    def test_weak_kleene_excluded_middle(self):
        ...
        assert result.satisfiable == True, "p ∨ ¬p should be satisfiable"
    ...
    def test_priest_many_valued_completeness(self):
        ...
        for formula in satisfiable_formulas:
            ...
            if result.satisfiable:
                assert len(result.models) > 0, f"Satisfiable formula {formula} should have models"
        ...
        for formula in unsatisfiable_formulas:
            ...
            assert len(result.models) == 0, f"Unsatisfiable formula {formula} should have no models"
...
class TestFittingTableauMethods:
    ...
    def test_fitting_model_extraction_efficiency(self):
        ...
        for model in result.models:
            assert isinstance(model.valuations, dict), "Each model should have valuations"
    ...
    def test_fitting_subsumption_elimination(self):
        ...
        assert found_p_only_model, "Should find model where p satisfies the whole formula"
    ...
    def test_smullyan_alpha_beta_classification_correctness(self):
        ...
        for alpha in alpha_formulas:
            ...
            if result.satisfiable:
                ...
                assert len(result.models) >= 1, f"Alpha formula {alpha} should have models"
        ...
        for beta in beta_formulas:
            ...
            if result.satisfiable:
                ...
                assert len(result.models) >= 1, f"Beta formula {beta} should have models"
...
class TestPerformanceOptimization:
    ...
    def test_alpha_rule_prioritization(self):
        ...
        assert result.satisfiable, "Formula should be satisfiable"
        assert (end_time - start_time) < 0.01, "Should solve quickly with alpha prioritization"
    ...
    def test_branch_selection_optimization(self):
        ...
        assert (end_time - start_time) < 0.1, "Should solve efficiently with branch optimization"
    ...
    def test_subsumption_elimination_effectiveness(self):
        ...
        assert result.satisfiable, "Formula should be satisfiable"
        assert (end_time - start_time) < 0.01, "Should solve efficiently with subsumption"
...
class TestInferencePatterns:
    ...
    def test_invalid_inference_detection(self):
        ...
        for inf_str in invalid_inferences:
            ...
            assert len(result.countermodels) > 0, f"Should provide countermodel for '{inf_str}'"
