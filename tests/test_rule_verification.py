...
import pytest
from wkrq import parse, SignedFormula, t, f, e, m, n
...
class TestConjunctionRules:
    ...
    def test_m_conjunction_meaningful_branching(self):
        ...
        # Branch 1: t: (P ∧ Q)
        assert rule.conclusions[0] == [SignedFormula(t, conj)]
        # Branch 2: f: (P ∧ Q)
        assert rule.conclusions[1] == [SignedFormula(f, conj)]
    ...
    def test_n_conjunction_nontrue_branching(self):
        ...
        # Branch 1: f: (P ∧ Q)
        assert rule.conclusions[0] == [SignedFormula(f, conj)]
        # Branch 2: e: (P ∧ Q)
        assert rule.conclusions[1] == [SignedFormula(e, conj)]
...
class TestNegationRules:
    ...
    def test_m_negation_general_elimination(self):
        ...
        assert wkrq_rule.conclusions[0] == [SignedFormula(n, p)]
    ...
    def test_n_negation_general_elimination(self):
        ...
        assert wkrq_rule.conclusions[0] == [SignedFormula(m, p)]
...
class TestQuantifierRules:
    ...
    def test_f_restricted_forall_fresh_constant(self):
        ...
        fresh_gen = lambda: Constant("c_fresh")
    ...
    def test_t_restricted_exists_fresh_constant(self):
        ...
        fresh_gen = lambda: Constant("c_new")
        rule = get_wkrq_rule(signed, fresh_gen)
    ...
    def test_quantifier_reapplication(self):
        ...
        rule1 = get_wkrq_rule(signed, lambda: Constant("c1"), ["a", "b"], used_constants=set())
        assert rule1.instantiation_constant == "a"
        
        # Second application with 'b' (a is now used)
        rule2 = get_wkrq_rule(signed, lambda: Constant("c1"), ["a", "b"], used_constants={"a"})
        assert rule2.instantiation_constant == "b"
        
        # Third application needs fresh constant
        rule3 = get_wkrq_rule(signed, lambda: Constant("c_fresh"), ["a", "b"], used_constants={"a", "b"})
...
class TestACrQDifferences:
    ...
    def test_atomic_negation_handling(self):
        ...
        assert acrq_rule.conclusions[0] == [SignedFormula(f, p)]
...
class TestBranchingStructure:
    ...
    def test_branch_count_verification(self):
        ...
        test_cases = [
            ...
            (SignedFormula(m, create_conjunction(p, q)), "m-conjunction", 2),
            (SignedFormula(n, create_conjunction(p, q)), "n-conjunction", 2),
        ]
        ...
        for signed, expected_name, expected_branches in test_cases:
            ...
            assert len(rule.conclusions) == expected_branches, \
    ...
    def test_branch_independence(self):
        ...
        for i, branch in enumerate(rule.conclusions):
            assert isinstance(branch, list), f"Branch {i} should be a list"
            # Modifying one branch shouldn't affect others
            original = branch.copy()
            branch.append(SignedFormula(t, PredicateFormula("R", [])))
            for j, other_branch in enumerate(rule.conclusions):
                if i != j:
                    assert SignedFormula(t, PredicateFormula("R", [])) not in other_branch
...
class TestSignOperations:
    ...
    def test_sign_negation_wkrq(self):
        ...
        from wkrq.signs import negate_sign
        
        assert negate_sign(t) == f
        assert negate_sign(f) == t
        assert negate_sign(e) == e
        assert negate_sign(m) == n
        assert negate_sign(n) == m
        assert negate_sign(v) == v
