...
class TestACrQCLI:
    ...
    def test_acrq_bilateral_contradiction(self):
        ...
        stdout, stderr, code = self.run_wkrq("--mode=acrq", "Human(alice) & ¬Human(alice)")
        assert code == 0
    ...
    def test_acrq_bilateral_syntax(self):
        ...
        stdout, stderr, code = self.run_wkrq("--mode=acrq", "--syntax=bilateral", "Human*(alice)")
        assert code == 0
    ...
    def test_acrq_mixed_syntax(self):
        ...
        stdout, stderr, code = self.run_wkrq("--mode=acrq", "--syntax=mixed", "Human*(alice) & ¬Robot(bob)")
        assert code == 0
    ...
    def test_acrq_inference(self):
        ...
        stdout, stderr, code = self.run_wkrq("--mode=acrq", "Human(alice), ¬Robot(alice) |- Human(alice)")
        assert code == 0
    ...
    def test_acrq_models(self):
        ...
        stdout, stderr, code = self.run_wkrq("--mode=acrq", "--models", "Human(alice) | Robot(bob)")
        assert code == 0
...
class TestACrQSemantics:
    ...
    def test_knowledge_gap_representation(self):
        ...
        from wkrq.acrq_parser import parse_acrq_formula, SyntaxMode
    ...
    def test_paraconsistent_reasoning(self):
        ...
        from wkrq.acrq_parser import parse_acrq_formula, SyntaxMode
        from wkrq.acrq_tableau import ACrQTableau
        from wkrq.signs import SignedFormula, T, F
    ...
    def test_bilateral_predicate_independence(self):
        ...
        from wkrq.acrq_semantics import ACrQInterpretation, ACrQEvaluator
        from wkrq.formula import BilateralPredicateFormula, Constant
        from wkrq.semantics import TRUE, FALSE
        ...
        # Test evaluations
        human = BilateralPredicateFormula("Human", [Constant("alice")], is_negative=False)
        human_star = BilateralPredicateFormula("Human", [Constant("alice")], is_negative=True)
        robot = BilateralPredicateFormula("Robot", [Constant("bob")], is_negative=False)
        robot_star = BilateralPredicateFormula("Robot", [Constant("bob")], is_negative=True)
