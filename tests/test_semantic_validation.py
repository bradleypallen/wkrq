...
class TestWeakKleeneTruthTables:
    ...
    def test_conjunction_truth_table_literature_compliance(self):
        ...
        assert conjunction[(FALSE, UNDEFINED)] == UNDEFINED  # Key difference from Strong Kleene
        assert conjunction[(UNDEFINED, FALSE)] == UNDEFINED  # Key difference from Strong Kleene
...
class TestContagiousUndefinedPropagation:
    ...
    def test_complex_undefined_propagation(self):
        ...
        assert result.satisfiable, "Undefined formulas should be satisfiable under N sign"


class TestNonClassicalTautologyBehavior:
    """Test that classical tautologies are not tautologies in weak Kleene logic."""
    
    def test_law_of_excluded_middle_not_tautology(self):
        """Test that p ∨ ¬p is NOT a tautology in weak Kleene logic.
        
        Reference: Literature states 'In three-valued logic, there are no tautologies.'
        """
        p = Formula.atom("p")
        excluded_middle = p | ~p
        
        # In classical logic, this would be a tautology
        # In weak Kleene logic, it's satisfiable under F sign (meaning it can be false)
        result_f = solve(excluded_middle, F)
        assert result_f.satisfiable, "p ∨ ¬p should be satisfiable under F (can be false)"
        
        # Also satisfiable under N sign (can be undefined)  
        result_n = solve(excluded_middle, N)
        assert result_n.satisfiable, "p ∨ ¬p should be satisfiable under N (can be undefined)"
        
        # This demonstrates non-classical behavior
        assert not valid(excluded_middle), "p ∨ ¬p should NOT be valid in weak Kleene logic"
        
    def test_law_of_non_contradiction_not_tautology(self):
        """Test that ¬(p ∧ ¬p) is NOT a tautology in weak Kleene logic."""
        p = Formula.atom("p")
        non_contradiction = ~(p & ~p)
        
        # Should be satisfiable under F sign (can be false)
        result_f = solve(non_contradiction, F)
        assert result_f.satisfiable, "¬(p ∧ ¬p) should be satisfiable under F"
        
        # Should be satisfiable under N sign (can be undefined)
        result_n = solve(non_contradiction, N)  
        assert result_n.satisfiable, "¬(p ∧ ¬p) should be satisfiable under N"
        
        assert not valid(non_contradiction), "¬(p ∧ ¬p) should NOT be valid in weak Kleene logic"
        
    def test_material_conditional_properties(self):
        """Test material conditional behavior in weak Kleene logic."""
        p, q = Formula.atoms("p", "q")
        
        # Classical tautologies that should fail in weak Kleene
        ...
        for formula in tautology_candidates:
            # Each should be satisfiable under F and N signs
            result_f = solve(formula, F)
            result_n = solve(formula, N)
            
            assert result_f.satisfiable, f"{formula} should be satisfiable under F"
            assert result_n.satisfiable, f"{formula} should be satisfiable under N"
            assert not valid(formula), f"{formula} should NOT be valid in weak Kleene logic"
...
class TestFergusonFourSignSystem:
    ...
    def test_epistemic_vs_truth_functional_distinction(self):
        ...
        # F:(p ∧ ¬p) tests if contradiction can be false - this should be unsatisfiable
        # since a contradiction cannot be determinately false
        assert not result_f.satisfiable, "F:(p ∧ ¬p) should be unsatisfiable"
        
        # Epistemic signs: express uncertainty, should be satisfiable
        result_m = solve(contradiction, M)
        assert result_m.satisfiable, "M:(p ∧ ¬p) should be satisfiable (epistemic uncertainty)"
    ...
    def test_sign_interaction_principles(self):
        ...
        assert entails([sf.formula for sf in premises], conclusion.formula), \
...
class TestTableauSoundnessProperties:
    ...
    def test_contradiction_detection_soundness(self):
        ...
        result_m = solve(contradiction, M)
        result_n = solve(contradiction, N)
        assert result_m.satisfiable, "Contradictions should be satisfiable under M (epistemic)"
        assert result_n.satisfiable, "Contradictions should be satisfiable under N (undefined)"
...
class TestSQLNullLogicCompatibility:
    ...
    def test_sql_null_or_semantics(self):
        ...
        assert disjunction[(FALSE, UNDEFINED)] == UNDEFINED  # FALSE OR NULL = NULL (matches SQL)
        assert disjunction[(UNDEFINED, UNDEFINED)] == UNDEFINED  # NULL OR NULL = NULL (matches SQL)
