...
from wkrq import solve, parse, t, f, e
from wkrq.semantics import WeakKleeneSemantics, TRUE, FALSE, UNDEFINED
from wkrq.formula import PredicateFormula, CompoundFormula, PropositionalAtom
...
class TestSemanticValidation:
    ...
    def test_implication(self):
        ...
        for p_val, q_val in product([TRUE, FALSE, UNDEFINED], repeat=2):
            ...
            if p_val == FALSE or q_val == TRUE:
                assert semantic_result == TRUE, \
                    f"P={p_val}, Q={q_val}: Should be TRUE"
            elif p_val == TRUE and q_val == FALSE:
                assert semantic_result == FALSE, \
                    f"P={p_val}, Q={q_val}: Should be FALSE"
            else:
                assert semantic_result == UNDEFINED, \
                    f"P={p_val}, Q={q_val}: Should be UNDEFINED"
    ...
    def test_excluded_middle_fails(self):
        ...
        result = solve(formula, f)  # Try to find counterexample
        assert result.satisfiable, "Should find case where P ∨ ¬P is not true"
