...
import time
import statistics
from wkrq import Formula, solve, check_inference, parse_inference, T, F, M, N
...
class TestBasicPerformanceBenchmarks:
    ...
    def test_negation_alpha_rule_performance(self):
        ...
        assert avg_time < 1.0, f"Average time {avg_time:.3f}ms should be < 1ms (alpha rule priority)"
    ...
    def test_contradiction_detection_performance(self):
        ...
        assert avg_time < 1.0, f"Average time {avg_time:.3f}ms should be < 1ms (fast closure)"
...
class TestScalabilityBenchmarks:
    ...
    def test_wide_conjunction_scaling(self):
        ...
        for size in sizes:
            ...
            assert result.satisfiable, f"Conjunction of {size} atoms should be satisfiable"
        
        # Check that time growth is reasonable (not exponential)
        for i in range(1, len(times)):
            ratio = times[i] / times[i-1]
            assert ratio < 7.0, f"Time ratio {ratio:.2f} should not exceed 7x for reasonable scaling"
        
        # Final size should still be fast
        assert times[-1] < 50.0, f"Large conjunction time {times[-1]:.3f}ms should be < 50ms"
    ...
    def test_cnf_formula_performance(self):
        ...
        assert result.total_nodes < 200, f"Node count {result.total_nodes} should be reasonable"
    ...
    def test_branching_factor_control(self):
        ...
        assert elapsed < 300.0, f"Branching formula time {elapsed:.3f}ms should be < 300ms"
        assert result.total_nodes < 600, f"Node count {result.total_nodes} should be controlled"
...
class TestInferencePerformanceBenchmarks:
    ...
    def test_complex_inference_performance(self):
        ...
        assert elapsed < 10.0, f"Complex inference time {elapsed:.3f}ms should be < 10ms"
...
class TestOptimizationEffectiveness:
    ...
    def test_alpha_beta_prioritization_effectiveness(self):
        ...
        assert elapsed < 5.0, f"Alpha/beta prioritization time {elapsed:.3f}ms should be < 5ms"
        
        # Should have processed efficiently (low node count indicates good prioritization)
        assert result.total_nodes < 50, f"Node count {result.total_nodes} should be low with prioritization"
    ...
    def test_early_termination_effectiveness(self):
        ...
        assert elapsed < 0.5, f"Early termination time {elapsed:.3f}ms should be < 0.5ms"
    ...
    def test_contradiction_detection_effectiveness(self):
        ...
        assert not result.satisfiable, "Contradiction should be unsatisfiable"
        assert elapsed < 5.0, f"Fast contradiction detection time {elapsed:.3f}ms should be < 5ms"
    ...
    def test_subsumption_elimination_effectiveness(self):
        ...
        assert elapsed < 5.0, f"Subsumption elimination time {elapsed:.3f}ms should be < 5ms"
...
class TestMemoryEfficiency:
    ...
    def test_node_count_efficiency(self):
        ...
        assert result.total_nodes < 100, f"Node count {result.total_nodes} should be efficient"
        assert result.open_branches + result.closed_branches < 20, "Branch count should be reasonable"
    ...
    def test_model_extraction_efficiency(self):
        ...
        for model in result.models:
            ...
            assert len(model.constants) == 0, "No constants needed for propositional logic"
...
class TestRegressionPrevention:
    ...
    def test_baseline_performance_maintained(self):
        ...
        for name, formula, max_time_ms in test_cases:
            ...
            assert elapsed < max_time_ms, f"{name} regression: {elapsed:.3f}ms > {max_time_ms}ms"
